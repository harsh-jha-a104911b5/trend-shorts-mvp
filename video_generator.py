...
def generate_video(script: dict) -> str:
    ...
    # Export
    outpath = os.path.join(config.OUTPUT_DIR, f"short_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{script['angle']}.mp4")
...
def _render_frame(text: str, idx: int, bg_img: Image.Image | None, scenes: list[str]) -> Image.Image:
...
def _draw_centered_text(draw: ImageDraw, text: str, font: ImageFont, W: int, y_pos: int):
    ...
    # Shadow
    draw.multiline_text((x+4, y_pos+4), lines, font=font, fill="black", align="center")
    # Main
